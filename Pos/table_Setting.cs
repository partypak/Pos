...
namespace Pos
{
    ...
    public partial class table_Setting : Form
    {
        ...
        private void btn_tableSave_Click(object sender, EventArgs e)
        {
            ...
            {
                mysql.Open();
                string deleteQuery = string.Format("DELETE FROM `table`;");
                MySqlCommand deletecmd = new MySqlCommand(deleteQuery, mysql);
                deletecmd.ExecuteNonQuery();
                for (int i = 0; i < table.Length; i++)
                {
                    string hidden_check = "Y";
                    if (btn_Array[i].Visible==false)
                    {
                        hidden_check = "X";
                    }
                    string updateQuery = string.Format("INSERT INTO `table`(table_num, table_name, table_x, table_y, table_width, table_height,hidden) VALUES ('{0}', '{1}','{2}','{3}','{4}','{5}','{6}');",
                        i, btn_Array[i].Name, btn_Array[i].Location.X, btn_Array[i].Location.Y, btn_Array[i].Width, btn_Array[i].Height,hidden_check);
        ...
        private void table_Setting_FormClosing(object sender, FormClosingEventArgs e)
        {
            Application.Exit();
        }
    }
